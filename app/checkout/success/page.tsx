...
function CheckoutSuccessContent() {
  ...
  useEffect(() => {
    ...
    async function captureOrder() {
      try {
        ...
        // Clear the cart backup from sessionStorage on success
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-24 text-center">
      ...
      {orderDetails && (
        <dl className="mt-8 rounded-2xl border border-border bg-card p-6 text-left space-y-3 text-sm">
          <div className="flex justify-between">
            <dt className="text-muted-foreground">Número de transacción</dt>
            <dd className="font-mono text-foreground text-xs">{orderDetails.transactionId}</dd>
          </div>
          <div className="flex justify-between">
